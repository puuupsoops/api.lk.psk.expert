...
function deleteOffersPosition(instance){
	...
	let del = new Promise((resolve,reject) => {
		...
		currentOrder.Positions[index].deleteOffer(id);
		resolve(true);
});
...
function deleteOrderPosition(instance){
	let product_id = $(instance).parent().parent().find('i').text();
	console.log(product_id);
	currentOrder.deletePosition(product_id);
...
function drawOrderItem(instance, data,iterator){
    ...
    $(instance).append(`
	<div class="order-list-item" data-index-position="${iterator}">
	<i style="display: none">${data.product.Id}</i>
        <div class="order-list-row order-list-main-row">
			<div class="order-list-elem">
            ${iterator+1}
            <div class="table-arrow"></div>
            </div>
            <div class="order-list-elem">${data.product.Name}</div>
            <div class="order-list-elem">${Number.parseFloat(data.totalPrice).toFixed(2)} ₽</div>
            <div class="order-list-elem">${data.totalAmount}</div>
            <div class="order-list-elem">${Number.parseFloat(data.totalCost).toFixed(2)} ₽</div>
    ...
    drawOrderSubItem(instance, data, iterator);
}
...
function drawOrderTable(){
	...
	$('.order-list-main-row').click(function() {
		...
		drawProtection(currentOrder.Positions[Number.parseInt( $(this).find('.order-list-elem').first().text()) - 1].product);

		clearSlider();
		addInSlider(currentOrder.Positions[Number.parseInt( $(this).find('.order-list-elem').first().text()) - 1].product.ImagesList);
		createSlider();
